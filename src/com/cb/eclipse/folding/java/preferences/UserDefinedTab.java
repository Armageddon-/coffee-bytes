...
public class UserDefinedTab implements PreferencesTab {
	...
	private void createTypeAdder(Composite parent) {
		...
		label.setText("Name");
		final Text name = new Text(group, SWT.BORDER | SWT.SINGLE);
		
		final Button collapse = new Button(group, SWT.CHECK);
		...
		button.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				boolean collapseIt = collapse.getSelection();
				FoldingPlugin.getJavaDomain().getUserDefinedSettings().addType(userInput, true, collapseIt);
			}
		});
