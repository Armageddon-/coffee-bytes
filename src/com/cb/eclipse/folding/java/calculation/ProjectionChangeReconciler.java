...
public class ProjectionChangeReconciler {
	private ILog L = FoldingPlugin.getDefault().getLog();
...
//			printAnnotations(currentAnnotations);
			Map rebuiltAnnotations = reconstructAnnotations(input);
//			printAnnotations(rebuiltAnnotations);
			...
			for(Iterator annotIt = model.getAnnotationIterator(); annotIt.hasNext(); ) {
				ProjectionAnnotation pa = (ProjectionAnnotation) annotIt.next();
				if(pa.isCollapsed()) {
					model.collapse(pa);
				}
			}
	...
	private void printAnnotations(Map annotations) {
		for(Iterator iter = annotations.entrySet().iterator(); iter.hasNext();) {
	...
	private class MinimumLineCountFilter extends PositionFilter {
		...
		boolean shouldFilter(Map allPositions, EnhancedPosition position) 
				throws BadLocationException {
...
//						"cb.eclipse.folding", 
//						382, 
//						"document in null in MinimumLineCountFilter shouldFilter"
//						, null));
				return false;
			}
